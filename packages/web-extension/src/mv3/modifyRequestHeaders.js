 ...
 */
export async function modifyRequestHeaders(urlPrefix, headers, inspectUrls = [], handler) {
  // MV3: no blocking webRequest, use DNR modifyHeaders instead.
  if (!chrome?.declarativeNetRequest?.updateDynamicRules) {
    console.warn('[mv3] declarativeNetRequest unavailable; skip modifyRequestHeaders', urlPrefix);
    return;
  }

  if (handler) {
    console.warn('[mv3] modifyRequestHeaders(handler) is not supported; ignoring handler');
  }
  ...
  const ruleId = stableRuleId(`hdr:${urlPrefix}:${Object.keys(headers).sort().join(',')}`);
